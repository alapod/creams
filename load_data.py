...
def login(chrome):
    ...
    chrome.find_element("xpath", "//*[@class='loginUsername form-control']").send_keys(
        ""
    )
    chrome.find_element("xpath", "//*[@class='loginPassword form-control']").send_keys(
        "-"
    )
    chrome.find_element(
        "xpath", "//*[@class='login-submit button d-inline-block']"
    ).click()
...
class Moisturizer:
    ...
    def __repr__(self):
        return f"Moisturizer {self.name} by {self.brand}\nrating {self.rating}\ningredients {self.ingredients}\n\n"

    def save(self):
        pass  # marshmallow

    def csv_friendly(self, sep="\uE000"):
        return (
            f"{self.brand}{sep}{self.name}{sep}{self.rating}{sep}{self.ingredients}\n"
        )
    ...
    def get_ingredients(self, browser):
        ...
        try:
            ...
            browser.find_element(
                "xpath", '//*[@data-event="show_ingredients_clicked"]'
            ).click()
            sleep(2)
            ...
            page = browser.page_source
            root = etree.fromstring(page, parser=html.HTMLParser())
            self.ingredients = root.xpath(
                '//*[@class="form-control edit-ingredients"]'
            )[0].text.replace("\n", " ")
        except:
...
def txt_to_csv(filename):
    links = pd.read_csv(filename, sep="\uE000", engine="python")
